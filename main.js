...
async function result() {
  if (display.value=="") {
    ...
    let msj = { method: 'POST', headers: { "Content-type": "application/json;charset=UTF-8" }, body: `{"items":"${display.value}" }` };
    let res = await fetch('http://localhost:5000/calcular', msj)
    res = await res.json();
    display.value=res.items
  }
